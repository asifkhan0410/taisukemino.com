  ...
  const files =
    otherLocale === ""
      ? getAllFilesRecursively(prefixPaths).filter(
          (path) => (path.match(/\./g) || []).length === 2 // this has to be 2 but not 1 because the repo name contains one "."
        )
...
export async function getFileBySlug(type, slug, otherLocale = "", locale) {
  const [mdxPath, mdPath] =
    otherLocale === ""
      ? [
          path.join(root, "data", type, `${locale.concat('/', slug)}.mdx`),
          path.join(root, "data", type, `${slug}.md`),
        ]
      : [
          path.join(root, "data", type, `${locale.concat('/', slug)}.${otherLocale}.mdx`),
...
export async function getAllFilesFrontMatter(folder, otherLocale) {
  ...
  // Check if the file exist in the otherlocale. If not, fallback to defaultLanguage
